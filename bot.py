...
def send_help(message):
    bot.reply_to(message,
        ...
        "/deepseek – проверить подключение к DeepSeek")

@bot.message_handler(commands=['deepseek'])
...
def handle_photo(message):
    try:
        ...
        # ============ ПРОВЕРКА НА СПЕЦИФИЧНЫЕ ТЕСТЫ ============
        # Текстовый процессор (информатика)
        if "текстовой процессор" in text_lower or "параметрами абзаца" in text_lower:
            answer = "отступ, интервал"
            response = f"""✅ **Информатика 7 класс**  
📖 Тема: Текстовый процессор

📌 Вопрос: В текстовом процессоре основными параметрами абзаца являются...
...
💡 Ответ: **отступ, интервал**

📖 Объяснение:
Основные параметры абзаца в текстовом процессоре:
• **Отступ** — расстояние от края страницы до текста (красная строка)
• **Интервал** — расстояние между строками (междустрочный интервал)
• Поля и ориентация — это параметры страницы
• Гарнитура, размер, начертание — это параметры шрифта
"""
        ...
        if "селекция" in text_lower or "растений" in text_lower:
            response = """✅ **Биология 7 класс**  
📖 Тема: Селекция растений

📌 Вопрос: Выберите верные задачи селекции растений.
...
📋 Варианты:
  • повышение урожайности и качества культур ✓
  • разработка приёмов возделывания культурных растений ✗ (это агротехника)
  • разработка методов создания и совершенствования сортов ✓
  • наблюдение и анализ роста урожаев ✗ (это мониторинг)
  • создание сортов и гибридов растений с нужными человеку ✓

📖 Объяснение:
Селекция — это наука о создании новых сортов растений и пород животных.
Задачи селекции:
1. Повышение урожайности и качества культур
2. Создание новых сортов и гибридов
3. Разработка методов селекции

Не относятся к селекции:
• Разработка приёмов возделывания (агротехника)
• Наблюдение и анализ роста урожаев (мониторинг)
"""
            bot.edit_message_text(response, message.chat.id, processing_msg.message_id)
            os.remove(image_path)
            if os.path.exists(processed_path):
        ...
        # Австралия (география)
        if "австралия" in text_lower or "канберра" in text_lower:
            response = """✅ **География 7 класс**  
📖 Тема: Австралия

📌 Вопрос: Выберите верные утверждения об Австралии.

✅ Ответ: **Столица Австралии — Канберра**

📋 Варианты:
  • Основная часть населения проживает в городах на западном и юго-западном побережьях. ✗
  • Плотность населения в центральных районах Австралии очень высокая. ✗
  • Большая часть населения Австралии — потомки переселенцев из Африки. ✗
  • Столица Австралии — Канберра. ✓
  • Самый густонаселённый район Австралии — Западная Австралия. ✗

📖 Объяснение:
• **Столица Австралии — Канберра** (верно)
• Основная часть населения проживает на **юго-восточном** побережье
• Плотность населения в центральных районах **очень низкая** (пустыни)
• Большая часть населения — потомки переселенцев из **Европы**, а не Африки
• Самый густонаселённый район — **юго-восточное** побережье
"""
            bot.edit_message_text(response, message.chat.id, processing_msg.message_id)
            os.remove(image_path)
            if os.path.exists(processed_path):
        ...
        # Очищаем вопросы для DeepSeek
        clean_questions = []
        for q in questions:
            q_text = re.sub(r'[^\w\s.,;:!?()"\'\-]', '', q['question'])
            q_text = re.sub(r'\s+', ' ', q_text).strip()
            clean_options = []
            for opt in q.get('options', []):
                opt_clean = re.sub(r'[^\w\s.,;:!?()"\'\-]', '', opt)
        ...
        print(f"Очищенные вопросы: {clean_questions}")
